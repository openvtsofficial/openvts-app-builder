...
export function ProjectStudio({ project }: { project: StudioProject }) {
  ...
  const [globalBusy, setGlobalBusy] = useState<{ busy: boolean; currentBuild?: { userEmail: string; type: string; progress: number } }>();
  ...
  const runBuild = async (type: BuildType) => {
    ...
    // Check if another user's build is running
    if (!DEMO_MODE) {
      try {
        const statusResponse = await fetch("/api/builds/status");
