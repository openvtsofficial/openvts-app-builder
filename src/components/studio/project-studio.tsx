...
export function ProjectStudio({ project }: { project: StudioProject }) {
  ...
  return (
    <div className="enter-up mx-auto max-w-[1320px]">
      <div className="flex flex-col gap-5 border-b border-[var(--line)] pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div><Link href="/dashboard" className="mb-3 inline-flex items-center gap-2 text-[10px] font-semibold text-[var(--muted)] hover:text-[var(--foreground)]"><ArrowLeft className="size-3" />Applications</Link><div className="flex items-center gap-3"><span className="grid size-11 place-items-center overflow-hidden rounded-xl border border-[var(--line)] bg-[var(--surface)]">{draft.iconPreviewUrl ? <img src={draft.iconPreviewUrl} alt="" className="size-full object-cover" /> : <Smartphone className="size-4" />}</span><div><div className="flex items-center gap-2"><h1 className="text-[22px] font-bold tracking-[-.04em]">{draft.name}</h1><Badge tone={dirty ? "warning" : "neutral"}>{dirty ? "Unsaved" : `Revision ${baseline.configurationRevision}`}</Badge></div><p className="mt-1 text-[9px] text-[var(--muted)]">Android and iOS application studio</p></div></div></div>
        <Button size="lg" variant={dirty ? "primary" : "secondary"} loading={saving} onClick={() => void save()}>{saved || !dirty ? <Check className="size-4" /> : <Save className="size-4" />}{saved ? "Changes saved" : dirty ? "Save changes" : "Saved"}</Button>
      </div>

      <div className="mt-6 grid gap-5 2xl:grid-cols-[minmax(470px,0.86fr)_minmax(540px,1.14fr)]">
...
function SectionTitle({ title, copy }: { title: string; copy: string }) { return <div><h2 className="text-[18px] font-bold tracking-[-.035em]">{title}</h2><p className="mt-1.5 text-[10px] text-[var(--muted)]">{copy}</p></div>; }
function PlatformPanel({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) { return <div className="rounded-[14px] border border-[var(--line)] bg-[var(--surface-elevated)] p-4"><div className="mb-4 flex items-center gap-3"><span className="grid size-8 place-items-center rounded-[9px] bg-[var(--surface)] shadow-sm">{icon}</span><p className="text-[11px] font-bold">{title}</p></div><div className="grid gap-4">{children}</div></div>; }
...
function BuildProgress({ build, log, onDownload, onClose }: { build: StudioBuild; log: string[]; onDownload: () => void; onClose: () => void }) {
  ...
  return (
    <section className="mt-4 overflow-hidden rounded-xl border border-[var(--line)] bg-[var(--surface-elevated)] shadow-[var(--shadow-card)]">
      <div className="grid gap-3 p-4 sm:grid-cols-[auto_1fr_auto] sm:items-center">
        <span className={cn("grid size-10 place-items-center rounded-[10px]", build.status === "SUCCEEDED" ? "bg-[var(--success-soft)] text-[var(--success)]" : build.status === "FAILED" ? "bg-[var(--danger-soft)] text-[var(--danger)]" : "bg-[var(--accent-soft)]")}>
          {build.status === "SUCCEEDED" ? <Check className="size-4" /> : build.status === "FAILED" ? <AlertCircle className="size-4" /> : <LoaderCircle className="size-4 animate-spin" />}
        </span>
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <p className="text-[11px] font-bold">{buildLabels[build.type]}</p>
            ...
            {running ? <span className="text-[8px] text-[var(--muted)]">{elapsed}s elapsed</span> : null}
          </div>
          <p className="mt-1.5 text-[9px] text-[var(--muted)]">{build.currentStage}</p>
          ...
          <div className="mt-1.5 flex items-center justify-between">
            ...
            {running && build.etaSeconds ? <span className="flex items-center gap-1 text-[8px] text-[var(--muted)]"><Clock3 className="size-2.5" />~{Math.ceil(build.etaSeconds)}s remaining</span> : null}
          </div>
        </div>
        <div className="flex items-center gap-2">
          {build.status === "SUCCEEDED" ? <Button size="sm" onClick={onDownload}><Download className="size-3.5" />Download</Button> : done ? <Button size="sm" variant="secondary" onClick={onClose}>Close</Button> : null}
        </div>
      </div>

      <div className="border-t border-[var(--line)]">
        <button type="button" onClick={() => setShowLog(!showLog)} className="flex w-full items-center justify-between px-4 py-2.5 text-[8px] font-bold text-[var(--muted)] hover:text-[var(--foreground)]">
          <span>Build output · {logLines.length} lines</span>
          <span>{showLog ? "▾" : "▸"}</span>
        </button>
        {showLog ? (
          <div ref={logRef} className="max-h-48 overflow-auto border-t border-[var(--line)] bg-[#0d0d0d] px-4 py-3 font-mono text-[9px] leading-[18px] text-[#a8b1ba]">
